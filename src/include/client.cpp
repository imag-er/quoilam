...
bool quoilam::Client::send_size(const uint32_t& size)
{
    // 先发送4字节的request长度数据
...
bool quoilam::Client::send_bytes(const quoilam::Byte* data, const uint32_t& nbytes)
{
    // 发送request
...
bool quoilam::Client::recv_bytes(Byte* buffer, const uint32_t& nbytes)
{
    // 接受response
...
void quoilam::Client::disconnect()
{
    if (owned_socket > 0)
        close(owned_socket);
